...
export async function populateWaitlists() {
    ...
    if(allRequests.length === 0) {
        ...
        allRequests.forEach(element => {
            ...
            if(hospitalIndex >= 0) {
                console.log(`Placing ${element._id.toHexString()} into the waitlist at position ${requestOrderIndex}`);
                allWaitlists[hospitalIndex].initInsert(element, requestOrderIndex);
            }
        })
