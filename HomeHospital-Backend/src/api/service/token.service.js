...
export const checkAccessToken = async (req, res, next) => {
  ...
  if (accessToken && refreshToken /* && cookieAccessToken && cookieRefToken*/) {
  ...
  if (allTokensPresent) {
    try {
      const validAccessToken = jwt.verify(accessToken, ACCESSTOKEN_TEST_SECRET); //jwt.verify returns the entire token. By accessing valid.email, we get only the payload of the token, the user's email
